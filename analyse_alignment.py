...
class analyse_alignment:
    ...
    def afasta_parse(self, file):
        ...
        Returns two sequences of equal length
        """
        try:
    ...
    def visualize_residue_conservation(self, filtered_alignments, braf_reference_seq=None, 
        ...
        Args:
            ...
            braf_reference_seq (str): Reference BRAF sequence for labeling
        ...
        # Add colored regions and parentheses
        paren_y = 1.01
        paren_height = 0.08
        
        for start, end, color, label in color_regions:
            # Add colored rectangle above bars
            ...
            # Add parentheses
            arc_left = Arc(
                xy=(start, paren_y + paren_height/2),
        ...
        ax.set_xticks(x[::5])
        if braf_reference_seq:
            ax.set_xticklabels([f"{braf_reference_seq[i]}{i+1}" for i in x[::5]], 
                              rotation=60, fontsize=7)
        else:
            ax.set_xticklabels([str(i+1) for i in x[::5]], rotation=60, fontsize=7)
        ...
        plt.ylabel("Conservation Percentage", fontsize=14)
        plt.title(f"Residue Conservation across {total_alignments} Structures", fontsize=16)
    ...
    IS THIS NECESSARY?
    '''
    def filter_alignments_by_motifs(self, alignments):
        """
        Filter alignments to keep only those with DFG and APE motifs present.
        
        Args:
            alignments (list): List of analyse_alignment objects
            
        Returns:
            tuple: (new_aligned, bad) where new_aligned contains valid alignments
                   and bad contains alignments without required motifs
        """
        new_aligned = []
        bad = []
        
        for alignment in alignments:
            if alignment.seq1.find("DFG") == -1 or alignment.seq1.find("APE") == -1:
                bad.append(alignment)
            else:
                new_aligned.append(alignment)
        
        print(f"{len(bad)} / {len(alignments)} structures don't have DFG and APE motifs.")
        print(f"Continuing with {len(new_aligned)} structures")
        
        # Save new_aligned for future use
        with open('new_aligned_data.pkl', 'wb') as f:
            p.dump(new_aligned, f)
        print(f"Saved {len(new_aligned)} filtered structures to 'new_aligned_data.pkl'")
        
        return new_aligned, bad
    ...
    def filter_alignments_by_gaps(self, alignments):
        ...
        Returns:
            tuple: (validated_alignments, aligning_segs, counts, lengths)
        """
        ...
        for alignment in alignments:
            ...
            if validation_result['valid']:
                counter += 1
                dfg_index = validation_result['dfg_index']
                ape_index = validation_result['ape_index']
                
                length = ape_index - dfg_index
                counts.setdefault(length, []).append((alignment.name, dfg_index, ape_index))
                validated_alignments.append(alignment)
                
                aligning_segs.setdefault(alignment.name, (
                    validation_result['dfg_seq2'],
                    validation_result['ape_seq2'],
                    alignment.seq2[dfg_index:ape_index+3]
                ))
                
                lengths.append(len(alignment.seq2[dfg_index:ape_index+3]))
        
        print(f"{counter} structures with proper DFG and APE alignment (no gaps)")
        
        # Save filtered results
        with open('filtered_aligned_data.pkl', 'wb') as f:
            filtered_data = {
                'new_aligned2': validated_alignments,
                'aligning_segs': aligning_segs,
                'counts': counts,
                'lengths': lengths
            }
            p.dump(filtered_data, f)
        print(f"Saved {len(validated_alignments)} final filtered structures to 'filtered_aligned_data.pkl'")
        
        return validated_alignments, aligning_segs, counts, lengths
    ...
    def filter_and_save_structures_with_motifs(self, target_dir="Results/activation_segments/reconstructed_mustang_filtered/", source_dir=None):
        """
