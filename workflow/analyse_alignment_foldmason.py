...
class analyse_alignment:
    ...
    # ---------------------------------------------------------------------
    # FoldMason multi-alignment helpers (used by truncation / downstream steps)
    # ---------------------------------------------------------------------

    @staticmethod
    def iter_fasta_records(fasta_file):
        """
        Stream a FASTA/AFasta file as (header, sequence) tuples.

        This avoids reading the entire file into memory (FoldMason `msa_3di.fa` can be large).
        """
        header = None
        seq_parts = []
    ...
    @classmethod
    def load_alignment_dict(cls, fasta_file):
        """
        Load a multi-structure alignment FASTA as a dict: {id: aligned_sequence}.

        The dict key is the first token of the FASTA header (i.e. up to first whitespace).
        """
        seq_by_id = {}
        for header, seq in cls.iter_fasta_records(fasta_file):
            key = header.split()[0] if header else header
            if key:
                seq_by_id[key] = seq
        return seq_by_id

    @staticmethod
    def pdb_to_msa_id(pdb_path: str) -> str:
        """
        Map '.../1APM_E_reconstructed_aligned.pdb' -> '1APM_E' (FoldMason msa header key).
        """
    ...
    @staticmethod
    def reference_indices_for_resnum_range(reference_residues, start_resnum=528, end_resnum=697):
        """
        Convert reference residue labels like 'ALA-528' into reference (0-based) indices
        for residues with residue-number in [start_resnum, end_resnum].
        """
        idxs = []
        for i, label in enumerate(reference_residues or []):
            try:
                resnum = int(str(label).split("-")[-1])
            except Exception:
                continue
            if int(start_resnum) <= resnum <= int(end_resnum):
                idxs.append(i)
        return idxs

    @staticmethod
    def target_indices_aligned_to_reference_set(ref_aln: str, target_aln: str, wanted_ref_set):
        """
        Return target residue indices (0-based, ungapped) aligned to desired reference indices.
        Only positions where BOTH ref and target are not gaps are considered "aligned".
        """
        ref_i = -1
        tar_i = -1
        tar_indices = []

        for c1, c2 in zip(ref_aln, target_aln):
            if c1 != "-":
                ref_i += 1
            if c2 != "-":
                tar_i += 1
            if c1 != "-" and c2 != "-" and ref_i in wanted_ref_set:
                tar_indices.append(tar_i)

        # preserve order but remove duplicates
        seen = set()
        out = []
        for x in tar_indices:
            if x not in seen:
                out.append(x)
                seen.add(x)
        return out
