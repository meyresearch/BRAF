...
# Import utilities (support both package import and direct script execution)
try:
...
class PCAAnalyzer:
    ...
    def plot_scores(self, folder_for_residues: str, figure_title: str = "PCA scores",
        ...
        # Historically this code assumed 27 CA atoms (n_features=81). Our fitting step can now output
        # a variable number of CA atoms per dataset (e.g., median CA count), so infer n_atoms from
        # the PCA model dimensionality.
        n_features = int(self.pca_model.components_.shape[1])
        if n_features % 3 != 0:
            raise ValueError(
                f"PCA model has {n_features} features, which is not divisible by 3. "
                "Expected Cartesian coords flattened as [n_atoms * 3]."
            )
        n_atoms = n_features // 3
        scores = self.pca_model.components_.reshape(-1, n_atoms, 3)[:n_comps]
        
        x_labels = utilities.braf_res(folder_for_residues)
        if len(x_labels) != n_atoms:
            # Fall back to generic labels rather than crashing on mismatched residue labels.
            # This can happen if the residue-label reference file differs from the PCA input.
            x_labels = [str(i) for i in range(n_atoms)]
        x_vals = list(range(n_atoms))
...
class ClusterAnalyzer:
    ...
    def plot_pca_cluster_and_activation_grid(
        self,
        results_by_N: dict,
        *,
        kincore_file: str = "Results/dunbrack_assignments/kinase_conformation_assignments.csv",
        cluster_grid_path: str = "pca_clustering_labels_ALL.png",
        activation_grid_path: str = "pca_activation_states_ALL.png",
        ncols: Optional[int] = None,
        nrows: Optional[int] = None,
        show: bool = True,
        title_prefix: str = "Anchored PCA",
    ) -> dict:
        """
        Create and save *combined* PCA plots across many N values.

        This is the "all N in one figure" companion to `plot_pca_cluster_and_activation()`.
        It produces two grid figures:
        - PCA PC1/PC2 colored by clustering labels
        - PCA PC1/PC2 colored by KinCore activation labels

        Args:
            results_by_N: Dict mapping N -> results dict (same schema as used by
                `plot_pca_cluster_and_activation()`).
            kincore_file: KinCore assignments CSV used to derive activation labels.
            cluster_grid_path: Output PNG path for the clustering grid.
            activation_grid_path: Output PNG path for the activation grid.
            ncols: Number of subplot columns.
            show: If True, display figures; else close them after saving.
            title_prefix: Prefix used in figure titles.

        Returns:
            Dict with output paths and Ns included.
        """
        ...
        items = [(N, results_by_N[N]) for N in sorted(results_by_N.keys(), key=_sort_key)]
        items = [(N, r) for (N, r) in items if r]
        if not items:
            raise ValueError("No non-empty results found in results_by_N.")

        n_panels = len(items)

        # Layout:
        # - if user supplies both nrows+ncols, use a fixed grid (e.g., 3 rows x 7 cols)
        # - otherwise fall back to a computed grid using ncols (default=3)
        if nrows is not None and ncols is not None:
            nrows = max(1, int(nrows))
            ncols = max(1, int(ncols))
            grid_capacity = nrows * ncols
            if n_panels > grid_capacity:
                raise ValueError(
                    f"Requested grid {nrows}x{ncols} can only hold {grid_capacity} panels, "
                    f"but got {n_panels} N values. Increase nrows/ncols or filter Ns."
                )
        else:
            ncols = 3 if ncols is None else int(ncols)
            ncols = max(1, ncols)
            nrows = int(np.ceil(n_panels / ncols))

        # -------------------------
        # Figure 1: clustering grid
        # -------------------------
        ...
        bounds = list(range(self.n_clusters + 1))
        norm_bound = BoundaryNorm(bounds, ncolors=self.n_clusters, clip=True)
        cluster_mappable = None

        for idx, (N, results) in enumerate(items):
            r = idx // ncols
            c = idx % ncols
            ax = axes1[r][c]
            ...
            if cluster_labels is None:
                raise ValueError(
                    f"N={N}: No cluster labels found (expected 'hierarchical_labels' or 'spectral_labels')."
                )
            ...
            ax.set_title(f"N={N} (n={len(projections)})")
        ...
        fig1.suptitle(f"{title_prefix} – Clustering labels (all N)", y=1.02)
        # Leave room on the right for a shared colorbar (keeps it out of the grid).
        fig1.tight_layout(rect=[0.0, 0.0, 0.90, 0.95])

        # Shared discrete colorbar (placed outside the grid, not between panels)
        if cluster_mappable is not None:
            cax = fig1.add_axes([0.92, 0.15, 0.02, 0.70])  # [left, bottom, width, height]
            tick_positions = [x + 0.5 for x in range(self.n_clusters)]
            cbar = fig1.colorbar(
                cluster_mappable,
                cax=cax,
            ...
            cbar.ax.set_yticklabels([f"Cluster {i}" for i in range(self.n_clusters)])
            cbar.set_label("Cluster ID")
        os.makedirs(os.path.dirname(cluster_grid_path) or ".", exist_ok=True)
        fig1.savefig(cluster_grid_path, dpi=300, bbox_inches="tight")
        if show:
            plt.show()
        else:
            plt.close(fig1)
        ...
        activation_cmap = ListedColormap(["red", "green"])  # Inactive=red, Active=green
        activation_bounds = [0, 1, 2]
        activation_norm = BoundaryNorm(activation_bounds, ncolors=2, clip=True)
        activation_mappable = None
        ...
        for idx, (N, results) in enumerate(items):
            ...
            activation_labels, _ = self.load_kincore_labels(
                structure_names,
                kincore_file=kincore_file,
            )
            ...
            if activation_mappable is None:
                activation_mappable = sc
            ...
            active_n = int(np.sum(activation_labels == 1))
            inactive_n = int(np.sum(activation_labels == 0))
            ax.set_title(f"N={N} (A={active_n}, I={inactive_n})")
        ...
        fig2.suptitle(f"{title_prefix} – Activation states (all N)", y=1.02)
        # Leave room on the right for the shared colorbar (keeps it out of the grid).
        fig2.tight_layout(rect=[0.0, 0.0, 0.90, 0.95])

        if activation_mappable is not None:
            cax = fig2.add_axes([0.92, 0.15, 0.02, 0.70])  # [left, bottom, width, height]
            tick_positions = [0.5, 1.5]
            cbar = fig2.colorbar(
                activation_mappable,
                cax=cax,
            ...
            cbar.ax.set_yticklabels(["Inactive", "Active"])
            cbar.set_label("Activation State")
        os.makedirs(os.path.dirname(activation_grid_path) or ".", exist_ok=True)
        fig2.savefig(activation_grid_path, dpi=300, bbox_inches="tight")
        if show:
            plt.show()
        else:
            plt.close(fig2)
        ...
        return {
            "cluster_grid_path": cluster_grid_path,
            "activation_grid_path": activation_grid_path,
            "Ns_included": [N for (N, _) in items],
            "n_panels": n_panels,
            "nrows": nrows,
            "ncols": ncols,
        }
    ...
    @staticmethod
    def discover_n_values_for_dunbrack_merge(
        *,
        n_values=None,
        results_by_N: Optional[dict] = None,
        fitted_dirs_by_N: Optional[dict] = None,
        base_dir: str = "Results/activation_segments",
    ) -> list:
        """
        Discover N values for PCA/Dunbrack integration from available notebook/runtime artifacts.
        """
        if n_values:
            return sorted({int(x) for x in n_values})
        if isinstance(results_by_N, dict) and results_by_N:
            return sorted({int(x) for x in results_by_N.keys()})
        if isinstance(fitted_dirs_by_N, dict) and fitted_dirs_by_N:
            return sorted({int(x) for x in fitted_dirs_by_N.keys()})
    ...
    def integrate_dunbrack_with_pca_clusters_for_all_n(
        self,
        *,
        n_values=None,
        results_by_N: Optional[dict] = None,
        ...
        """
        Run Dunbrack/PCA merge for each discovered N value and return results by N.
        """
        n_list = self.discover_n_values_for_dunbrack_merge(
            n_values=n_values,
            results_by_N=results_by_N,
            fitted_dirs_by_N=fitted_dirs_by_N,
            base_dir=base_dir,
        )
        if not n_list:
            raise NameError(
                "No N values found for Dunbrack merge. Run PCA first or provide n_values/results_by_N/fitted_dirs_by_N."
            )

        os.makedirs(output_dir, exist_ok=True)
        merged_by_N = {}
        for N in n_list:
            out_dir = os.path.join(base_dir, f"pca_anchored_{N}")
            ...
            if not os.path.exists(pca_labels_file):
                raise FileNotFoundError(
                    f"Missing PCA labels file: {pca_labels_file}. Re-run PCA for N={N} first."
                )

            merged_output_csv = os.path.join(output_dir, f"pca_dunbrack_merged_N{N}.csv")
            print(f"\n=== Dunbrack merge (N={N}) ===")
            out = self.integrate_dunbrack_with_pca_clusters(
                pca_labels_file=pca_labels_file,
                dunbrack_assignments_csv=dunbrack_assignments_csv,
                prefix_len=prefix_len,
                merged_output_csv=merged_output_csv,
                print_tables=print_tables,
                print_percentages=print_percentages,
            )
            merged_by_N[int(N)] = out

        return merged_by_N

    @staticmethod
    def discover_n_values_for_cluster_activity(
        *,
        n_list=None,
        n_values=None,
        merged_dir: str = "Results/dunbrack_assignments",
    ) -> list:
        """
        Discover N values for cluster-vs-activity analysis.
        """
        if n_list:
            return sorted({int(x) for x in n_list})
        if n_values:
            return sorted({int(x) for x in n_values})

        ns = []
        for p in glob.glob(os.path.join(merged_dir, "pca_dunbrack_merged_N*.csv")):
            m = re.search(r"pca_dunbrack_merged_N(\d+)\.csv$", os.path.basename(p))
            if m:
                ns.append(int(m.group(1)))
        return sorted(set(ns))

    def analyze_cluster_vs_activity_for_all_n(
        self,
        *,
        n_list=None,
        n_values=None,
        ...
        """
        Run cluster-vs-activity analysis for each N merged CSV and return results by N.
        """
        _n_list = self.discover_n_values_for_cluster_activity(
            n_list=n_list,
            n_values=n_values,
        ...
        if not _n_list:
            raise NameError(
                "No N values found for cluster-vs-activity analysis. Run Dunbrack merge first."
            )

        cluster_vs_activity_by_N = {}
        for N in _n_list:
            merged_csv = os.path.join(merged_dir, f"pca_dunbrack_merged_N{N}.csv")
            if not os.path.exists(merged_csv):
                raise FileNotFoundError(f"Missing merged CSV: {merged_csv}")

            print(f"\n=== Cluster vs activity (N={N}) ===")
            out = self.analyze_cluster_vs_activity_status(
                merged_csv=merged_csv,
                print_tables=print_tables,
                print_percentages=print_percentages,
                print_enrichment=print_enrichment,
                enrichment_threshold_pct=enrichment_threshold_pct,
            )
            cluster_vs_activity_by_N[int(N)] = out

        return cluster_vs_activity_by_N
...
class PCAWorkflow:
    ...
    def run_full_analysis(
        ...
        # `output_prefix` is often passed with a directory component (e.g. "Results/.../my_run").
        # This workflow constructs filenames like f"scores_{output_prefix}.png". If `output_prefix`
        # contains '/', that naive concatenation yields invalid paths like:
        #   "scores_Results/.../my_run.png"
        # We therefore split it into:
        # - output_dir: where to write outputs
        # - prefix_base: the filename base without directories
        output_dir = os.path.dirname(str(output_prefix)) or "."
        prefix_base = os.path.basename(str(output_prefix)) or "pca"
        os.makedirs(output_dir, exist_ok=True)
        ...
        fig_scores, axes_scores = pca.plot_scores(
            structures_path,
            figure_title=f"PCA Scores - {prefix_base}"
        )
        scores_file = os.path.join(output_dir, f"scores_{prefix_base}.png")
        plt.savefig(scores_file, bbox_inches='tight', dpi=300)
        plt.close(fig_scores)
        print(f"Saved PCA scores plot: {scores_file}")
        
        # Save PC1 and PC2 values
        pca.save_pc_values(0, structure_names, os.path.join(output_dir, f"pc1_{prefix_base}.txt"))
        pca.save_pc_values(1, structure_names, os.path.join(output_dir, f"pc2_{prefix_base}.txt"))
        ...
        if perform_hierarchical:
            ...
            fig_hier, ax_hier = cluster_hier.plot_clusters(
                projections,
                labels_hier,
                main_title=f"Hierarchical Clustering - {prefix_base}",
                explained_var_ratios=pca.get_explained_variance_ratios()
            )
            
            hier_file = os.path.join(output_dir, f"cluster_{prefix_base}_hierarchical.png")
            fig_hier.savefig(hier_file, dpi=300)
            plt.close(fig_hier)
            print(f"Saved hierarchical clustering plot: {hier_file}")
            
            # Save cluster labels
            cluster_labels_file_hier = os.path.join(
                output_dir, f"cluster_labels_{prefix_base}_hierarchical.txt"
            )
        ...
        if perform_spectral:
            ...
            fig_spec, ax_spec = cluster_spec.plot_clusters(
                projections,
                labels_spec,
                main_title=f"Spectral Clustering - {prefix_base}",
                explained_var_ratios=pca.get_explained_variance_ratios()
            )
            
            spec_file = os.path.join(output_dir, f"cluster_{prefix_base}_spectral.png")
            fig_spec.savefig(spec_file, dpi=300)
            plt.close(fig_spec)
            print(f"Saved spectral clustering plot: {spec_file}")
            
            # Save cluster labels
            cluster_labels_file_spec = os.path.join(
                output_dir, f"cluster_labels_{prefix_base}_spectral.txt"
            )
