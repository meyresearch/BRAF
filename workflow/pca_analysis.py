...
class PCAAnalyzer:
    ...
    def fit_transform(self, file_list: List[str]) -> Tuple[PCA, np.ndarray]:
        ...
        atom_counts = {}
        for f in file_list:
            n_atoms = md.load(f).n_atoms
            atom_counts.setdefault(n_atoms, []).append(f)

        if len(atom_counts) > 1:
            summary = ", ".join(
                f"{n} atoms ({len(paths)} files)" for n, paths in sorted(atom_counts.items())
            )
            raise ValueError(
                f"PCA requires the same number of atoms in every structure, but found: {summary}. "
                "Run the cubic-spline Fitting step first and point PCA at "
                "Results/activation_segments/fitted/ (not raw CA_segments)."
            )

        file_list = atom_counts[next(iter(atom_counts))]
        n_atoms = next(iter(atom_counts))
        if len(file_list) <= 10:
            for f in file_list:
                print("  ", os.path.basename(f))
        else:
            print(
                f"  ({os.path.basename(file_list[0])} … {len(file_list)} structures, "
                f"{n_atoms} atoms each)"
            )
...
class ClusterAnalyzer:
    ...
    @staticmethod
    def normalize_structure_id(name: str) -> str:
        """
        Map PCA / fitted names to KinCore ``pdb_code`` style IDs.

        Examples:
          ``1A9U_A_aligned.pdb`` -> ``1A9U_A``
          ``1A9U_A_aligned`` -> ``1A9U_A``
          ``1A9U_A`` -> ``1A9U_A``
        """
        stem = os.path.splitext(os.path.basename(str(name)))[0]
        for suffix in (
            "_reconstructed_aligned",
            "_reconstructed",
            "_aligned",
            "_truncated",
        ):
    ...
    def load_kincore_labels(self, structure_names: List[str], 
        ...
        Args:
            structure_names: List of structure names to match (e.g. ``1A9U_A_aligned``)
        ...
        for _, row in kincore_df.iterrows():
            pdb_code = str(row['pdb_code']).strip()
            description = row['conformation_description']
            label = 1 if 'Active' in str(description) else 0
            activation_map[pdb_code] = label
            pdb_file = str(row.get('pdb_file', '') or '')
            if pdb_file:
                activation_map[os.path.splitext(os.path.basename(pdb_file))[0]] = label
        ...
        for name in structure_names:
            candidates = [
                ...
                self.normalize_structure_id(name),
            ]
            matched = None
            for key in candidates:
                if key in activation_map:
                    matched = activation_map[key]
                    break
            if matched is None:
                labels.append(0)  # default inactive
        ...
        if unmatched:
            print(
                f"Warning: {len(unmatched)}/{len(structure_names)} structures not found in KinCore file; "
                f"defaulting those to inactive. Examples: {unmatched[:5]}"
            )
        else:
            n_active = int(sum(1 for x in labels if x == 1))
            n_inactive = int(sum(1 for x in labels if x == 0))
            print(
                f"KinCore activation labels matched for all {len(structure_names)} structures "
    ...
    def plot_pca_cluster_and_activation(
        ...
        *,
        ...
        if isinstance(results, pd.DataFrame):
            raise TypeError(
                "Expected PCA results dict from PCAWorkflow.run_full_analysis(), "
                "but got a pandas DataFrame. A later cell likely overwrote `results` "
                "(for example DunbrackWorkflow.run()). Use `pca_results` instead."
            )
        if not isinstance(results, dict):
            raise TypeError(
                f"Expected PCA results dict, got {type(results).__name__}."
            )
    ...
    def plot_cluster_vs_activation_state_heatmap(
        ...
        *,
        ...
        if merged_clean.empty:
            state_counts = merged["activation_state"].value_counts().to_dict()
            raise ValueError(
                "No Active/Inactive structures found for the heatmap. "
                f"Activation-state counts: {state_counts}. "
                "This usually means KinCore assignments are all 'unknown' — run Dunbrack on "
                "full aligned kinase chains (not loop-only fitted PDBs), then regenerate "
                "kinase_conformation_assignments.csv and pca_dunbrack_merged.csv."
            )

        activation_crosstab = pd.crosstab(merged_clean[cluster_column], merged_clean["activation_state"])
        if activation_crosstab.empty:
            raise ValueError(
                "Activation-state crosstab is empty; check ClusterLabel and Dunbrack merge columns."
            )
    ...
    def plot_cluster_vs_activation_state_grouped_bar(
        self,
        *,
        merged_csv: str = "Results/dunbrack_assignments/pca_dunbrack_merged.csv",
        cluster_column: str = "ClusterLabel",
        description_column: str = "conformation_description",
        output_png: str = "Results/dunbrack_assignments/pca_activation_grouped_bar.png",
        figsize: tuple = (6, 4),
        show: bool = True,
        title: str = "PCA Cluster vs Activation State",
        print_saved_message: bool = True,
    ) -> dict:
        """
        Grouped bar chart of PCA cluster vs activation state counts.

        Each cluster is a group on the x-axis with side-by-side bars for
        Active and Inactive counts, making the composition of each cluster
        immediately readable without the confusion-matrix connotation of a
        heatmap.
        """
        merged = pd.read_csv(merged_csv)
        ...
        activation_crosstab = pd.crosstab(
            merged_clean[cluster_column], merged_clean["activation_state"]
        )

        fig, ax = plt.subplots(figsize=figsize)
        activation_crosstab.plot(kind="bar", ax=ax, edgecolor="black", width=0.7)

        for container in ax.containers:
            ax.bar_label(container, fmt="%d", fontsize=9, padding=2)

        ax.set_title(title, fontsize=12, fontweight="bold")
        ax.set_xlabel("PCA Cluster", fontsize=10)
        ax.set_ylabel("Count", fontsize=10)
        ax.set_xticklabels(ax.get_xticklabels(), rotation=0)
        ax.legend(title="Activation State")
        fig.tight_layout()
        ...
        if print_saved_message:
            print(f"Saved grouped bar chart to {output_png}")
