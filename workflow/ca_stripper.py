...
class CAStripper:
    ...
    def process_directory(self, input_dir, output_dir):
        ...
        for pdb_file in tqdm(pdb_files, desc="Processing PDB files"):
            ...
            out_fp = os.path.join(output_dir, os.path.basename(pdb_file))
            if os.path.exists(out_fp):
                skipped_existing += 1
                print(f"Skipping (already stripped exists): {out_fp}")
                continue
        ...
        print(f"{'#'*80}")
        print(
            f"Successfully processed {successful_count}/{len(pdb_files)} structures "
...
class OutlierStripper:
    ...
    def filter_by_distance(self, max_distance=3.0):
        ...
        # Filter structures where the maximum terminal distance is <= cutoff.
        # (Equivalent to both first/last <= cutoff, but more robust given we already compute max_distance.)
        filtered_df = self.distances_df[
            (self.distances_df["max_distance"] <= max_distance)
        ].copy()
    ...
    def filter_by_distance_percentile(self, discard_fraction=0.25):
        """
        Filter structures by discarding the worst fraction of terminal-CA distances.

        We rank structures by `max_distance` (max of first/last terminal CA distances to the reference),
        then discard the top `discard_fraction` (largest distances).

        Parameters:
        -----------
        discard_fraction : float, default=0.25
            Fraction in [0,1) of structures to discard as "worst" (largest max_distance).

        Returns:
        --------
        tuple[pd.DataFrame, float]
            (filtered_df, cutoff_distance_angstrom)
        """
        if self.distances_df is None:
            raise ValueError("No distance results available. Run calculate_distances() first.")

        f = float(discard_fraction)
        if not (0.0 <= f < 1.0):
            raise ValueError(f"discard_fraction must be in [0, 1); got {discard_fraction}")

        s = pd.to_numeric(self.distances_df["max_distance"], errors="coerce")
        s = s.dropna()
        if len(s) == 0:
            raise ValueError("No valid max_distance values available to compute percentile cutoff.")

        # Keep the best (1 - f) fraction => cutoff at quantile (1 - f)
        q = 1.0 - f
        cutoff = float(np.quantile(s.values, q))

        filtered_df = self.filter_by_distance(max_distance=cutoff)

        print(f"Percentile-based distance cutoff: discard worst {f*100:.1f}% by max_distance")
        print(f"Computed cutoff (keep max_distance <=): {cutoff:.3f} Å (quantile={q:.3f})")

        return filtered_df, cutoff

    def filter_by_distance_keep_n(self, keep_n=2523):
        """
        Filter structures by keeping exactly the best-N by terminal-CA distance.

        We rank structures by `max_distance` (max of first/last terminal CA distances to the reference),
        then keep the smallest `keep_n` distances and discard the rest.

        Notes
        -----
        - Structures with NaN `max_distance` are always discarded (cannot be ranked).
        - If `keep_n` is larger than the number of valid-distance structures, we keep all valid-distance
          structures (final count may be < keep_n).

        Parameters
        ----------
        keep_n : int, default=2523
            Target number of structures to keep (best by smallest max_distance).
        ...
        try:
            ...
            raise ValueError(f"keep_n must be an integer; got {keep_n!r}")

        if keep_n <= 0:
            raise ValueError(f"keep_n must be > 0; got {keep_n}")

        # Work only with valid distances; NaNs are not rankable and should be removed.
        ranked = self.distances_df.copy()
        ranked["max_distance"] = pd.to_numeric(ranked["max_distance"], errors="coerce")
        ranked = ranked.dropna(subset=["max_distance"])

        if len(ranked) == 0:
            raise ValueError("No valid max_distance values available to rank structures.")

        ranked = ranked.sort_values("max_distance", ascending=True).reset_index(drop=True)

        if keep_n >= len(ranked):
            cutoff = float(ranked["max_distance"].max())
            filtered_df = self.filter_by_distance(max_distance=cutoff)
            print(
                f"Target-N distance filtering requested keep_n={keep_n}, "
                f"but only {len(ranked)} structures had valid distances. Keeping all valid-distance structures."
            )
            print(f"Computed cutoff (keep max_distance <=): {cutoff:.3f} Å")
            return filtered_df, cutoff

        kept = ranked.head(keep_n)
        kept_files = set(kept["pdb_file"].values)
        cutoff = float(kept["max_distance"].max())

        # Return exactly those kept files (do not rely on cutoff alone; ties at cutoff could exceed keep_n).
        filtered_df = self.distances_df[self.distances_df["pdb_file"].isin(kept_files)].copy()

        n_total_valid = len(ranked)
        n_removed = n_total_valid - len(kept)
        print(f"Target-N distance filtering: keep best {keep_n} by max_distance (of {n_total_valid} valid-distance structures)")
        print(f"Computed cutoff (max_distance of kept set): {cutoff:.3f} Å")
        print(f"Structures removed by target-N: {n_removed}")
    ...
    def analyze(self, ca_segments_dir, create_plots=True, clean_dir_name=None, 
                distance_cutoff=3.0, distance_discard_fraction=None, distance_keep_n=None, apply_distance_filter=None,
        ...
        distance_discard_fraction : float, optional
            If provided, use percentile-based screening instead of an absolute Å cutoff:
            discard the worst fraction (largest max_distance) and keep the rest.
        distance_keep_n : int, optional
            If provided, keep exactly this many structures with the smallest `max_distance`
            (discarding the worst distances until `distance_keep_n` remain).
            If both `distance_keep_n` and `distance_discard_fraction` are provided, `distance_keep_n` wins.
        ...
        if apply_distance_filter:
            ...
            # Filter by distance (either absolute cutoff or percentile-based)
            if distance_keep_n is not None:
                distance_filtered, applied_distance_cutoff = self.filter_by_distance_keep_n(
                    keep_n=distance_keep_n
                )
            elif distance_discard_fraction is not None:
                distance_filtered, applied_distance_cutoff = self.filter_by_distance_percentile(
                    discard_fraction=distance_discard_fraction
                )
            else:
                applied_distance_cutoff = float(distance_cutoff)
                distance_filtered = self.filter_by_distance(max_distance=applied_distance_cutoff)
        ...
        if create_plots:
            self.create_plots(ca_counts, clean_ca_counts, ca_segments_dir, self.k_factor, 
                            ...
                            distance_cutoff=applied_distance_cutoff if apply_distance_filter else distance_cutoff,
        ...
        results = {
            ...
            'distance_cutoff': applied_distance_cutoff if apply_distance_filter else None,
            'distance_discard_fraction': float(distance_discard_fraction) if (apply_distance_filter and distance_discard_fraction is not None) else None,
            'distance_keep_n': int(distance_keep_n) if (apply_distance_filter and distance_keep_n is not None) else None,
        ...
        if apply_distance_filter and self.distances_df is not None:
            ...
            distance_outliers = self.get_outliers(max_distance=applied_distance_cutoff)
            if len(distance_outliers) > 0:
                distance_outliers.to_csv(os.path.join(ca_segments_dir, f"distance_outliers_cutoff{applied_distance_cutoff}.csv"),
                                        index=False)
                print(f"Distance outliers saved to: distance_outliers_cutoff{applied_distance_cutoff}.csv")
    ...
    @staticmethod
    def discover_anchored_n_values(anchor_info=None, base_dir="Results/activation_segments"):
        """
        Discover anchored dataset suffix values N from anchor_info or on-disk folders.
        """
        if anchor_info is not None and isinstance(anchor_info, dict) and "anchor_sets" in anchor_info:
            try:
                return sorted(int(k) for k in anchor_info["anchor_sets"].keys())
            except Exception:
                pass

        ca_dirs = glob(os.path.join(base_dir, "CA_segments_anchored_*"))
        n_values = []
        for d in ca_dirs:
            m = re.search(r"CA_segments_anchored_(\d+)$", d.rstrip("/"))
            if m:
                n_values.append(int(m.group(1)))
        n_values = sorted(set(n_values))
        if not n_values:
            raise NameError(
                "No anchored datasets discovered. Expected folders like "
                "'Results/activation_segments/CA_segments_anchored_<N>/' or a valid anchor_info['anchor_sets']."
            )
        return n_values

    def analyze_anchored_datasets(
        self,
        *,
        anchor_info=None,
        n_values=None,
        ...
        distance_keep_n=None,
        distance_discard_fraction=None,
        distance_cutoff=3.0,
        clean_dir_prefix="CA_segments_final_cleaned_anchored",
    ):
        """
        Run outlier analysis across all anchored CA-segment datasets.

        Returns:
            dict[int, dict]: mapping N -> results returned by analyze()
        """
        if n_values is None:
            n_values = self.discover_anchored_n_values(anchor_info=anchor_info, base_dir=base_dir)
        else:
        ...
        for N in n_values:
            ...
            clean_name = f"{clean_dir_prefix}_{N}"

            print(f"\n=== Outlier filtering for N={N} ===")
            final_results = self.analyze(
                ca_segments_dir=ca_dir,
                create_plots=create_plots,
                apply_distance_filter=apply_distance_filter,
                distance_keep_n=distance_keep_n,
                distance_discard_fraction=distance_discard_fraction,
                distance_cutoff=distance_cutoff,
                clean_dir_name=clean_name,
            )
            final_results_by_N[int(N)] = final_results
            print(f"Final cleaned structures (N={N}) saved to: {final_results['clean_dir']}")

        return final_results_by_N