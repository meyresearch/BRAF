...
class ProteinReconstructor:
    ...
    def __init__(self, input_dir, full_pdb_dir, output_dir, max_gap_length=4,
        ...
        Args:
            ...
            max_gap_length: Maximum consecutive gap length allowed in the DFG–APE loop
            max_missing_residues: Maximum total missing residues allowed in the DFG–APE loop
        """
        self.input_dir = input_dir
        self.full_pdb_dir = full_pdb_dir
        self.output_dir = os.path.abspath(output_dir)
        self.max_gap_length = max_gap_length
        self.max_missing_residues = max_missing_residues
    ...
    def align_sequences(self, seqres_segment, atom_segment):
        ...
        Returns
        -------
        aligned_seqres, aligned_atom, max_gap_length, n_missing
        """
        aligned_seqres = ''
        aligned_atom = ''
        atom_index = 0
        ...
        n_missing = aligned_atom.count('-')
        return aligned_seqres, aligned_atom, max_gap_length, n_missing
    ...
    def process_structure(self, pdb_file):
        ...
        aligned_seqres, aligned_atom, max_gap_length, n_missing = self.align_sequences(
            seqres_segment, atom_segment
        )
        
        # Check if gap length / total missing are acceptable
        if max_gap_length > self.max_gap_length:
            ...
            return False
        if n_missing > self.max_missing_residues:
            print(
                f"Total missing residues ({n_missing}) exceeds maximum "
                f"({self.max_missing_residues}) for {pdb_id}_{chain_id}"
            )
            return False
        ...
        print(
            f"Processed {pdb_id}_{chain_id} - Max gap length: {max_gap_length}, "
            f"missing: {n_missing}"
        )
        return True

    def filter_by_max_gap(
        self,
        input_dir=None,
        output_dir=None,
    ):
        """
        Copy only structures whose DFG–APE loop passes consecutive-gap and
        total-missing cutoffs to *output_dir*.

        Excludes if ``max_gap > max_gap_length`` or
        ``n_missing > max_missing_residues``.

        Returns
        -------
        dict with keys: passed, excluded, n_passed, n_excluded, output_dir
        """
        ...
        print(
            f"Gap filter (max_gap={self.max_gap_length}, "
            f"max_missing={self.max_missing_residues}): "
            f"screening {len(pdb_files)} structures …"
        )

        passed, excluded = [], []

        for pdb_file in tqdm(pdb_files, desc="Checking gap lengths"):
            pdb_name = os.path.basename(pdb_file)
            stem = os.path.splitext(pdb_name)[0]
            parts = stem.split("_")
            if len(parts) < 2:
                print(f"  Unexpected filename (skipping): {pdb_name}")
                excluded.append(pdb_name)
                continue
            ...
            try:
                ...
                seqres_dfg = self.find_motif_indices(seqres_seq, "DFG")
                seqres_ape = self.find_motif_indices(seqres_seq, "APE")
                atom_dfg = self.find_motif_indices(atom_seq, "DFG")
                atom_ape = self.find_motif_indices(atom_seq, "APE")

                if None in (seqres_dfg, seqres_ape, atom_dfg, atom_ape):
                    excluded.append(pdb_name)
                    continue

                seqres_start = min(seqres_dfg, seqres_ape)
                seqres_end = max(seqres_dfg + 3, seqres_ape + 3)
                atom_start = min(atom_dfg, atom_ape)
                atom_end = max(atom_dfg + 3, atom_ape + 3)

                seqres_segment = seqres_seq[seqres_start:seqres_end]
                atom_segment = atom_seq[atom_start:atom_end]

                _, _, max_gap, n_missing = self.align_sequences(
                    seqres_segment, atom_segment
                )

                if max_gap > self.max_gap_length:
                    print(
                        f"  Excluded {pdb_id}_{chain_id}: "
                        f"gap {max_gap} > {self.max_gap_length}"
                    )
                    excluded.append(pdb_name)
                elif n_missing > self.max_missing_residues:
                    print(
                        f"  Excluded {pdb_id}_{chain_id}: "
                        f"missing {n_missing} > {self.max_missing_residues}"
                    )
                    excluded.append(pdb_name)
                else:
                    shutil.copy2(pdb_file, output_dir)
                    passed.append(pdb_name)

            except Exception as exc:
                print(f"  Error checking {pdb_name}: {exc}")
                excluded.append(pdb_name)
        ...
        if excluded:
            excl_path = os.path.join(output_dir, "gap_excluded.txt")
