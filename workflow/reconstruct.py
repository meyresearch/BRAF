...
class ProteinReconstructor:
    ...
    def process_structure(self, pdb_file):
        ...
        # Filenames in this workflow can be like:
        # - 1SYK_A.pdb
        # - 1SYK_A_aligned.pdb
        # - 1SYK_A_anything_else.pdb
        # so we only treat the first two underscore-separated fields as (pdb_id, chain_id).
        stem = os.path.splitext(pdb_name)[0]
        parts = stem.split('_')
        if len(parts) < 2:
            raise ValueError(
                f"Unexpected PDB filename format (expected at least 'PDBID_CHAIN*.pdb'): {pdb_name}"
            )
