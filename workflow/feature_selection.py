...
class FeatureSelection:
    ...
    def calculate_intra_structure_ca_distances(
        self,
        aligned_structures=None,
        pdb_directory: str = None,
        alignment_function=None,
    ) -> pd.DataFrame:
        """
        Compute Cα–Cα Euclidean distances between every pair of conserved residues.

        If ``self.structures`` is already populated (e.g. immediately after a
        ``calculate_intra_structure_distances`` call for the same set of PDB files),
        those already-loaded per-residue coordinates are reused and no arguments
        are required.  Otherwise ``aligned_structures``, ``pdb_directory``, and
        ``alignment_function`` must be supplied so the method can load structures.

        Cα atoms are always well-defined for standard amino acids; if a Cα is
        absent in a structure (malformed PDB) the pair is skipped and the entry
        will be absent in the returned DataFrame (treated as NaN during
        ``build_feature_matrix(use_median_imputation=True)``).

        Args:
            aligned_structures: List of alignment objects (required when
                self.structures is empty).
            pdb_directory: Directory with PDB files (required when
                self.structures is empty).
            alignment_function: Function that produces the alignment segment
                (required when self.structures is empty).

        Returns:
            DataFrame with columns matching ``intra_structure_df`` but
            containing Cα–Cα distances (Å).  The returned DataFrame is
            **not** stored on ``self``; the caller decides where to keep it.
        """
        if not self.structures:
            if aligned_structures is None or pdb_directory is None or alignment_function is None:
                raise ValueError(
                    "self.structures is empty — supply aligned_structures, "
                    "pdb_directory, and alignment_function so the method can "
                    "load structures from PDB files."
                )
            # ── Load structures (same as calculate_intra_structure_distances) ─
            all_pdb_files = [f for f in os.listdir(pdb_directory) if f.endswith('.pdb')]
            print("Loading structures for Cα distance computation…")
            self.structures = {}
            for alignment_obj in tqdm(aligned_structures, desc="Loading structures"):
                structure_name = alignment_obj.name
                short_name = structure_name[:6]
                matching_files = [f for f in all_pdb_files if f.startswith(short_name)]
                if not matching_files:
                ...
                try:
                    ...
                    for cons_idx, cons_residue_name in self.fully_conserved:
                        ...
                        if struct_res_idx is None or struct_res_idx >= struct.top.n_residues:
                            continue
                        res_atom_indices = [
                            atom.index for atom in struct.top.residue(struct_res_idx).atoms
                        ]
                        residue_coords[cons_idx] = {
                            'atom_indices': res_atom_indices,
                            'topology': struct.top.residue(struct_res_idx),
                    ...
                    print(f"Error loading {structure_name}: {e}")
            print(f"Loaded {len(self.structures)} structures for Cα distance computation")

        # ── Compute Cα–Cα distances from self.structures ─────────────────────
        distance_data = {
            'structure': [],
            'residue1_name': [],
            'residue1_position': [],
        ...
        for struct_name, residue_coords in tqdm(
            self.structures.items(), desc="Computing Cα–Cα distances"
        ):
            residue_positions = list(residue_coords.keys())
            if len(residue_positions) < 2:
                continue
            for pos1, pos2 in combinations(residue_positions, 2):
                res1_name = next(
                    (name for p, name in self.fully_conserved if p == pos1), f"Res{pos1}"
                )
                res2_name = next(
                    (name for p, name in self.fully_conserved if p == pos2), f"Res{pos2}"
                )
                res1_topo = residue_coords[pos1]['topology']
                res2_topo = residue_coords[pos2]['topology']
                res1_coords = residue_coords[pos1]['coordinates']
                res2_coords = residue_coords[pos2]['coordinates']

                # Find Cα atom index within residue atoms
                ca1_idx = next(
                    (i for i, a in enumerate(res1_topo.atoms) if a.name == 'CA'), None
                )
                ca2_idx = next(
                    (i for i, a in enumerate(res2_topo.atoms) if a.name == 'CA'), None
                )
                if ca1_idx is None or ca2_idx is None:
                    continue  # No CA found → entry absent → handled as NaN on build

                ca_dist = float(
                    np.linalg.norm(res1_coords[ca1_idx] - res2_coords[ca2_idx]) * 10.0
                )  # nm → Å
                distance_data['structure'].append(struct_name)
                distance_data['residue1_name'].append(res1_name)
                distance_data['residue1_position'].append(pos1)
                distance_data['residue2_name'].append(res2_name)
                distance_data['residue2_position'].append(pos2)
                distance_data['distance'].append(ca_dist)
    ...
    def filter_mutual_info_features(self, n_features: int = 300,
                                   plot_scores: bool = False,
                                   remove: bool = True) -> np.ndarray:
        """
        Filter features using Mutual Information, keeping top N features that
        best separate classes.

        Uses sklearn's mutual_info_classif (non-parametric; captures non-linear
        dependence). Median-imputes a copy of the feature matrix for scoring
        because MI does not accept NaNs.

        Args:
            n_features: Number of top features to keep (default 300)
            plot_scores: Whether to plot MI score distribution (default False)
            remove: If True, apply filtering to the feature matrix
        ...
        # Median-impute a copy for scoring (MI cannot handle NaNs)
        X_imp = self.feature_matrix.astype(float, copy=True)
        ...
        mi_scores = mutual_info_classif(X_imp, self.labels, random_state=42)
        ...
        if plot_scores:
            ...
            plt.subplot(1, 2, 1)
            plt.hist(mi_scores[mi_scores > 0], bins=50, edgecolor='black', color='skyblue')
            plt.axvline(mi_scores[top_indices[-1]], color='red', linestyle='--',
                       label=f'Selection threshold (MI={mi_scores[top_indices[-1]]:.4f})')
            plt.xlabel('Mutual Information', fontsize=12)
            plt.ylabel('Frequency', fontsize=12)
            plt.title('Distribution of MI scores', fontsize=14)
            plt.legend()
            plt.grid(True, alpha=0.3)

            plt.subplot(1, 2, 2)
            sorted_mi_scores = np.sort(mi_scores)[::-1]
            plt.plot(range(len(sorted_mi_scores)), sorted_mi_scores, linewidth=2)
            plt.axvline(n_to_select, color='red', linestyle='--',
                       label=f'Top {n_to_select} features')
            plt.axhline(mi_scores[top_indices[-1]], color='red', linestyle='--', alpha=0.5)
            plt.xlabel('Feature rank', fontsize=12)
            plt.ylabel('Mutual Information', fontsize=12)
            plt.title('Ranked MI scores', fontsize=14)
            plt.legend()
            plt.grid(True, alpha=0.3)
        ...
        print(f"\n📊 Mutual Information selection results:")
        print(f"   Features removed: {n_removed}")
        print(f"   Features remaining: {n_to_select}")
        ...
        if remove and n_removed > 0:
            ...
            print(f"\n✂️  Applied Mutual Information filtering")
            print(f"   New matrix shape: {self.feature_matrix.shape}")
    ...
    @staticmethod
    def summarize_filtering_and_export_anova_features(
        *,
        base_dir: str = ".",
        initial_matrix_csv: str = "feature_matrix.csv",
        corr_filtered_matrix_csv: str = "corr_filtered_feature_matrix.csv",
        anova_filtered_matrix_csv: str = "filtered_feature_matrix.csv",
        reference_pickle: str = "filtered_reference_data.pkl",
        summary_plot_png: str = "feature_counts_by_filtering_step.png",
        anova_features_csv: str = "anova_selected_features_full_list.csv",
        show_plot: bool = True,
    ) -> dict:
        """
        Build filtering-step summary and export parsed ANOVA-selected feature pairs.
        """
        base_dir = os.path.abspath(base_dir)
        steps = [
            ("Initial (all features)", initial_matrix_csv),
            ("After correlation filter", corr_filtered_matrix_csv),
            ("After ANOVA (selected)", anova_filtered_matrix_csv),
        ]
        ...
        for label, fname in steps:
            ...
            if os.path.exists(path):
                _df = pd.read_csv(path, index_col=0, nrows=1)
                rows.append({"step": label, "n_features": int(_df.shape[1]), "file": path})
            else:
                rows.append({"step": label, "n_features": None, "file": path})
        ...
        plot_df = summary_df.dropna(subset=["n_features"]).copy()
        if len(plot_df):
            plot_df["n_features"] = plot_df["n_features"].astype(int)
            plt.figure(figsize=(10, 4))
            plt.plot(plot_df["step"], plot_df["n_features"], marker="o")
            plt.ylabel("# features")
            plt.xticks(rotation=25, ha="right")
            plt.title("Features remaining after each feature-filtering step")
            plt.tight_layout()
            out_png = os.path.join(base_dir, summary_plot_png)
        ...
        anova_matrix_path = os.path.join(base_dir, anova_filtered_matrix_csv)
        if not os.path.exists(anova_matrix_path):
            raise FileNotFoundError(f"Missing ANOVA-filtered matrix: {anova_matrix_path}")

        anova_cols = pd.read_csv(anova_matrix_path, index_col=0, nrows=1).columns.tolist()
        pair_re = re.compile(r"^(\d+)-(\d+)$")
        parsed = []
        bad_cols = []
        for c in anova_cols:
            m = pair_re.match(str(c).strip())
            if not m:
                bad_cols.append(c)
                continue
            a, b = int(m.group(1)), int(m.group(2))
            parsed.append({"feature": c, "pos1": a, "pos2": b})

        anova_features_df = pd.DataFrame(parsed)

        ref_pkl = os.path.join(base_dir, reference_pickle)
        if os.path.exists(ref_pkl):
            with open(ref_pkl, "rb") as f:
                data = pickle.load(f)
            fully_conserved = data.get("fully_conserved", [])
            pos_to_res = {int(pos): str(name) for (pos, name) in fully_conserved}
            anova_features_df["res1"] = anova_features_df["pos1"].map(pos_to_res)
            anova_features_df["res2"] = anova_features_df["pos2"].map(pos_to_res)

        out_csv = os.path.join(base_dir, anova_features_csv)
        anova_features_df.to_csv(out_csv, index=False)
        print(f"Saved ANOVA-selected feature list to: {out_csv}")
        print(f"ANOVA-selected features: {len(anova_features_df):,}")

        if bad_cols:
            print("WARNING: Some feature columns did not match 'i-j' format. Showing first 25:")
            print(bad_cols[:25])

        try:
            from IPython.display import display
            display(anova_features_df.head(20))
        except Exception:
            print(anova_features_df.head(20).to_string(index=False))

        return {
            "summary_df": summary_df,
            "anova_features_df": anova_features_df,
            "bad_columns": bad_cols,
            "summary_plot_png": out_png,
            "anova_features_csv": out_csv,
        }
