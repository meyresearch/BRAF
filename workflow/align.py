...
class Alignment:
    ...
    def find_pdbs(self, directory, verbose=True):
    ...
    def process_structure(self, pdb_file, output_dir, ref_name="6UAN_chainD", verbose=True):
        """
        ...
        Args:
            ...
            verbose: If False, suppress skip/save messages; RMSD before/after lines are always printed.
        """
        try:
            try:
            ...
            # rotation_matrix() does not centre its inputs, so pass centroid-subtracted coordinates.
            mob_com = mob_ends.center_of_mass()
            ref_com = ref_ends.center_of_mass()
            R, _rms_after_fit = mda_align.rotation_matrix(
                mob_ends.positions - mob_com, ref_ends.positions - ref_com
            )
    ...
    @classmethod
    def restore_multi_n_anchor_context(
        cls,
        *,
        reference_pdb: str = "6UAN_chainD.pdb",
        alignment_file: str = "Results/activation_segments/multi_aligned_foldmason/msa_3di.fa",
        conservation_npy: str = "Results/activation_segments/multi_aligned_foldmason/conservation.npy",
        reference_name: str = "6UAN_chainD",
        threshold: float = 0.90,
        n_values=range(6, 27),
        aligned_root: str = "Results/activation_segments",
        aligned_prefix: str = "aligned_anchor_",
        recompute_conservation_if_missing: bool = True,
        show_conservation_plot: bool = False,
    ) -> dict:
        """
        Rebuild notebook state needed by post-alignment cells without re-running
        ``process_anchored_alignment``.

        Loads/creates:
          - ``aligner`` (new ``Alignment`` instance)
          - ``reference_residues`` via ``braf_res()``
          - ``conservation`` from ``conservation_npy``, or recomputes from ``alignment_file``
          - ``anchor_info`` via ``compute_anchor_positions_from_conservation``
          - lightweight ``anchored_runs`` with ``kept`` lists inferred from existing
            ``aligned_anchor_{N}/*.pdb`` folders (for plot titles / counts)
        ...
        conservation = None
        if conservation_npy and os.path.exists(conservation_npy):
            conservation = np.load(conservation_npy)
            print(f"Loaded conservation array: {conservation_npy} (len={len(conservation)})")
        elif recompute_conservation_if_missing:
            if not os.path.exists(alignment_file):
                raise FileNotFoundError(
                    f"Missing conservation cache ({conservation_npy}) and alignment file "
                    f"({alignment_file}). Run the FoldMason conservation cell first."
                )
            from workflow.analyse_alignment_foldmason import analyse_alignment

            print(f"Recomputing conservation from {alignment_file} (no FoldMason re-run)...")
            ...
            conservation_run = analyser.run_multi_alignment_conservation_analysis(
                ...
                output_plot=os.path.join(
                    os.path.dirname(alignment_file) or ".",
                    "multi_alignment_foldMason_conservation_resume.png",
                ),
                output_csv=os.path.join(
                    os.path.dirname(alignment_file) or ".",
                    "conserved_residues_70percent_resume.csv",
                ),
                show_plot=show_conservation_plot,
                verbose=False,
            )
            conservation = conservation_run["conservation"]
            os.makedirs(os.path.dirname(conservation_npy) or ".", exist_ok=True)
            np.save(conservation_npy, np.asarray(conservation, dtype=float))
            print(f"Saved conservation array for next resume: {conservation_npy}")
        else:
            raise FileNotFoundError(
                f"conservation_npy not found: {conservation_npy}. "
                "Run the FoldMason conservation cell or set recompute_conservation_if_missing=True."
            )
        ...
        anchored_runs = {}
        for N in sorted(anchor_info.get("anchor_sets", {}).keys()):
            out_dir = os.path.join(aligned_root, f"{aligned_prefix}{int(N)}")
            kept = sorted(glob(os.path.join(out_dir, "*.pdb"))) if os.path.isdir(out_dir) else []
            anchored_runs[int(N)] = {
                "kept": kept,
                "excluded": [],
                "out_dir": out_dir,
        ...
        n_with_pdbs = sum(1 for v in anchored_runs.values() if v["kept"])
        print(
            f"Restored anchor_info (N={min(anchored_runs) if anchored_runs else '?'}..{max(anchored_runs) if anchored_runs else '?'}); "
            f"{n_with_pdbs} aligned_anchor_* folders have PDBs on disk."
        )
        print("Skipping process_anchored_alignment (use the alignment cell only to regenerate structures).")
        ...
        return {
            ...
            "conservation_npy": conservation_npy,
        }
        ...
        *,
        ...
        if len(anchor_positions) == 0:
            raise ValueError(
                ...
                "This usually means compute_anchor_positions_from_conservation found no usable "
                "≥threshold flanks around DFG/APE (or N was too large). "
                "Try lowering the threshold, reducing max N, or inspect the conservation array."
            )
        ...
        for fp in iterator:
            ...
            R, rms_fit = mda_align.rotation_matrix(
                mob_atoms.positions - mob_com, ref_atoms.positions - ref_com
            )
    ...
    def setup_reference_structure(self, reference_pdb, ref_name="6UAN_chainD", verbose=True):
        """
        ...
        Args:
            ...
            verbose: If False, suppress informational prints (errors still print).
        """
        try:
            try:
                import MDAnalysis as mda
    ...
    def _primary_ca_resids(self, u, exclude_altloc=True):
        """Return sorted CA residue numbers from the primary chain."""
        ca = u.select_atoms("name CA")
        if len(ca) == 0:
            return []

        chain_ids = np.unique(ca.chainIDs)
        if len(chain_ids) > 1:
            chain = max(chain_ids, key=lambda ch: np.sum(ca.chainIDs == ch))
            ca = ca.select_atoms(f"chainID {chain}")

        resids = sorted({int(r) for r in ca.resids})
        if not exclude_altloc or not self.altloc_exclude:
            return resids

        kept = []
        for resid in resids:
            if len(self._select_ca_resids(u, [resid], exclude_altloc=True)) == 1:
                kept.append(resid)
        return kept

    def _termini_ca_resids(self, u, n_termini=3, exclude_altloc=True):
        """
        Return (first_resids, last_resids, anchor_resids) for the first/last *n* CA atoms.

        anchor_resids concatenates the N-terminal and C-terminal sets, dropping duplicate
        residue numbers when the segment is shorter than 2 * n_termini.
        """
        ca = self._primary_ca_resids(u, exclude_altloc=exclude_altloc)
        n_termini = int(n_termini)
        if n_termini < 1:
            raise ValueError("n_termini must be >= 1")
        if len(ca) < 3:
            return None, None, None

        first = ca[:n_termini]
        last = ca[-n_termini:]
        anchor = first + [r for r in last if r not in first]
        if len(anchor) < 3:
            return None, None, None
        return first, last, anchor

    def _ordered_ca_positions(self, u, resids, exclude_altloc=True):
        """Return CA coordinates for resids in the given order, or None if any resid is missing."""
        positions = []
        for resid in resids:
            atoms = self._select_ca_resids(u, [resid], exclude_altloc=exclude_altloc)
            if len(atoms) != 1:
                return None
            positions.append(atoms.positions[0])
        return np.asarray(positions, dtype=float)

    def setup_reference_ca_termini(self, reference_pdb, ref_name="6UAN_chainD", verbose=True):
        """
        Load the reference structure for CA-termini alignment.

        Anchor residue numbers are taken from each mobile structure at runtime and matched
        on the reference by PDB residue number, so the reference may be a full chain or a segment.
        """
        try:
            try:
                import MDAnalysis as mda
            ...
            if verbose:
                first, last, anchor = self._termini_ca_resids(self._ref_universe, n_termini=3, exclude_altloc=False)
                print(f"Reference CA termini (first 3): {first}")
                print(f"Reference CA termini (last 3):  {last}")
                print(f"Reference CA anchor resids:     {anchor}")
            return True
        except Exception as e:
            print(f"Error setting up reference structure: {e}")
            return False

    def process_structure_ca_termini(
        self,
        pdb_file,
        output_dir,
        n_termini=3,
        ...
        """
        Align a structure using CA termini as anchors.

        Works on CA-only coarse-grained segments where motif-based alignment is not possible.
        Reference anchor atoms are selected by matching the mobile termini residue numbers.

        Args:
            anchor_end: Which termini define the Kabsch fit — ``"both"`` (default),
                ``"first"``, or ``"last"``.
        """
        try:
            try:
            ...
            if self._ref_universe is None:
                raise ValueError(
                    "Reference not initialized. Call setup_reference_ca_termini(reference_pdb) first."
                )

            pdb_code = os.path.basename(pdb_file).split(".")[0]
            u = mda.Universe(pdb_file)

            first_resids, last_resids, anchor_resids = self._termini_ca_resids(
                u, n_termini=n_termini, exclude_altloc=True
            )
            if anchor_resids is None:
                if verbose:
                    print(f"Skipping {pdb_code}: fewer than 3 CA atoms available for termini alignment.")
                return

            anchor_end = str(anchor_end).lower()
            if anchor_end == "first":
                fit_resids = first_resids
            elif anchor_end == "last":
                fit_resids = last_resids
            elif anchor_end == "both":
                fit_resids = anchor_resids
            else:
                raise ValueError('anchor_end must be "both", "first", or "last"')

            mob_first = self._ordered_ca_positions(u, first_resids, exclude_altloc=True)
            mob_last = self._ordered_ca_positions(u, last_resids, exclude_altloc=True)
            mob_anchor = self._ordered_ca_positions(u, fit_resids, exclude_altloc=True)
            ref_first = self._ordered_ca_positions(self._ref_universe, first_resids, exclude_altloc=False)
            ref_last = self._ordered_ca_positions(self._ref_universe, last_resids, exclude_altloc=False)
            ref_anchor = self._ordered_ca_positions(self._ref_universe, fit_resids, exclude_altloc=False)

            if any(arr is None for arr in (mob_first, mob_last, mob_anchor, ref_first, ref_last, ref_anchor)):
                if verbose:
                    print(
                        f"Skipping {pdb_code}: could not match termini CA atoms on reference "
                        f"(anchors={anchor_resids})."
                    )
                return

            def _rms(pos_a, pos_b):
                if pos_a is None or pos_b is None or len(pos_a) != len(pos_b) or len(pos_a) == 0:
                    return np.nan
                return float(mda_rms.rmsd(pos_a, pos_b, center=False, superposition=False))

            rms_first_before = _rms(mob_first, ref_first)
            rms_last_before = _rms(mob_last, ref_last)
            rms_anchor_before = _rms(mob_anchor, ref_anchor)

            if verbose:
                print(
                    f"Before alignment RMSD for {pdb_code}: "
                    f"FIRST={rms_first_before}, LAST={rms_last_before}, ANCHORS={rms_anchor_before}"
                )

            # rotation_matrix() does not centre its inputs, so pass centroid-subtracted coordinates.
            mob_com = mob_anchor.mean(axis=0)
            ref_com = ref_anchor.mean(axis=0)
            R, _rms_after_fit = mda_align.rotation_matrix(
                mob_anchor - mob_com, ref_anchor - ref_com
            )
            ...
            mob_first_after = self._ordered_ca_positions(u, first_resids, exclude_altloc=True)
            mob_last_after = self._ordered_ca_positions(u, last_resids, exclude_altloc=True)
            mob_anchor_after = self._ordered_ca_positions(u, fit_resids, exclude_altloc=True)

            if verbose:
                print(
                    f"After alignment RMSD for {pdb_code}: "
                    f"FIRST={_rms(mob_first_after, ref_first)}, "
                    f"LAST={_rms(mob_last_after, ref_last)}, "
                    f"ANCHORS={_rms(mob_anchor_after, ref_anchor)}"
                )

        except Exception as e:
            print(f"Error processing {pdb_file}: {e}")

    def process_ca_termini_alignment(
        self,
        pdb_dir,
        reference_pdb,
        output_dir,
        ...
        """
        Align multiple structures using the first/last *n* CA atoms as anchors.

        Intended for CA-only coarse-grained segments where DFG/APE motif alignment fails.
        ...
        log_restore = []
        if quiet:
            for _name in ("MDAnalysis", "matplotlib", "PIL"):
                _lg = logging.getLogger(_name)
                log_restore.append((_lg, _lg.level))
                _lg.setLevel(logging.ERROR)

        warn_ctx = warnings.catch_warnings() if quiet else nullcontext()
        try:
            with warn_ctx:
                if quiet:
                ...
                if not self.setup_reference_ca_termini(reference_pdb, ref_name, verbose=verbose):
                    print("Failed to setup reference structure")
                    return

                fps = self.find_pdbs(pdb_dir, verbose=verbose)
                if not fps:
                ...
                iterator = tqdm(fps, desc=f"Aligning PDBs (CA {anchor_end})") if tqdm is not None else fps
                for fp in iterator:
                    if verbose:
                        print(f"Processing: {fp}")
                    self.process_structure_ca_termini(
                        fp,
                        output_dir,
                        n_termini=n_termini,
                        ref_name=ref_name,
                ...
                if verbose:
                    print("CA termini alignment processing completed successfully")

        except Exception as e:
            print(f"Error during alignment processing: {e}")
        finally:
            for _lg, _old in log_restore:
                _lg.setLevel(_old)
    
    def process_pymol_alignment(self, pdb_dir, reference_pdb, output_dir, ref_name="6UAN_chainD", quiet=True):
        """
        ...
        Args:
            ...
            ref_name: Name to assign to the reference structure
            quiet: If True (default), show only the tqdm bar and per-structure RMSD before/after lines.
                Suppresses reference residue listings, file counts, ``Processing:`` lines,
                skip reasons, save-path prints, completion banners, and dependency warnings.
                Use ``quiet=False`` for full diagnostic output.
        """
        verbose = not quiet
        log_restore = []
        if quiet:
        ...
        try:
            with warn_ctx:
                ...
                # Clear output so re-runs overwrite previous alignments
                if os.path.exists(output_dir):
                    shutil.rmtree(output_dir)
                os.makedirs(output_dir)

                if not self.setup_reference_structure(reference_pdb, ref_name, verbose=verbose):
                    print("Failed to setup reference structure")
                    return
                ...
                for fp in iterator:
                    if verbose:
                        print(f"Processing: {fp}")
                    self.process_structure(fp, output_dir, ref_name, verbose=verbose)
