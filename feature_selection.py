...
class FeatureSelection:
    ...
    def filter_correlated_features(self, correlation_threshold: float = 0.90, 
        ...
        print(f"\nCalculating correlation matrix for {n_features} features...")
        
        # Simple, robust correlation calculation
        # Works with or without NaN values
        correlation_matrix = np.zeros((n_features, n_features))
        np.fill_diagonal(correlation_matrix, 1.0)  # Diagonal is always 1
        
        if use_parallel and n_features > 50:
            try:
                ...
                def compute_correlations_chunk(i_start, i_end):
                    """Compute correlations for a chunk of features."""
                    local_results = []
                    for i in range(i_start, i_end):
                        for j in range(i+1, n_features):
                            # Find valid (non-NaN) indices for both features
                            valid_mask = ~(np.isnan(self.feature_matrix[:, i]) | np.isnan(self.feature_matrix[:, j]))
                            n_valid = np.sum(valid_mask)
                            
                            if n_valid > 1:
                                # Extract valid data and compute correlation
                                data_i = self.feature_matrix[valid_mask, i]
                                data_j = self.feature_matrix[valid_mask, j]
                                corr_matrix = np.corrcoef(data_i, data_j)
                                r = corr_matrix[0, 1]
                            else:
                                r = np.nan
                            
                            local_results.append((i, j, r))
                    return local_results
                
                # Split work into smaller chunks for better progress tracking
                chunk_size = max(10, n_features // (n_cores * 20))  # More chunks = more frequent updates
                chunks = [(i, min(i + chunk_size, n_features)) 
                         for i in range(0, n_features, chunk_size)]
                
                print(f"Processing {len(chunks)} chunks (each chunk ~{chunk_size} features)...")
                print(f"Total correlation pairs to compute: {(n_features * (n_features - 1)) // 2:,}")
                print("This may take several minutes for large matrices...")
                
                # Compute in parallel
                results = Parallel(n_jobs=n_cores)(
                    delayed(compute_correlations_chunk)(start, end) 
                    for start, end in tqdm(chunks, desc="Chunks completed", unit="chunk")
                )
                
                # Fill correlation matrix
                for chunk_results in results:
                    for i, j, r in chunk_results:
        ...
        if not use_parallel or n_features <= 50:
        ...
        for idx, feature_class in enumerate(correlated_classes):
            ...
            # Print only groups with multiple features
